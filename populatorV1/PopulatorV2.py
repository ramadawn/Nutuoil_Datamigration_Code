...
from item_cleaning_module import number_clean
...
incrementor = 0
for column in out_df_columns:
    poss_add_column = entry_clean("QUERY", column, clean_dictionary)
...
for file in file_list:
    ...
    for index, row in input_file_df.iterrows():
        ...
        for pos in range(len(row)):
            ...
            if pd.isnull(input_file_df.iloc[index][pos]) == False:
        ...
        for header in input_file_headers:
            ...
            if isinstance(data,str) == True:
                ...
                data, two_variables = entry_clean(data,header,clean_dictionary)

            else:

                data = number_clean(data,header,clean_dictionary)
            ...
            if key in map_dictionary:
                output_header = map_dictionary[key]
                output_df.iloc[output_dataframe_index][output_header] = data

                if two_variables == True:
                    output_df.iloc[output_dataframe_index][new_header] = second_string
